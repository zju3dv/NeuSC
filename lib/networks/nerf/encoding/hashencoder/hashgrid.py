...
class TriPlaneTcnn(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        import tinycudann as tcnn
        self.xy_plane = tcnn.Encoding(2, kwargs['tcnn_cfg'], dtype=torch.float)
        self.yz_plane = tcnn.Encoding(2, kwargs['tcnn_cfg'], dtype=torch.float)
        self.xz_plane = tcnn.Encoding(2, kwargs['tcnn_cfg'], dtype=torch.float)
        self.out_dim = self.xy_plane.n_output_dims * 3

    def forward(self, xyz, wbounds=torch.tensor([-1., -1., -1., 1., 1., 1.])):
        # inputs = torch.clamp(xyz, min=wbounds[:3], max=wbounds[3:6])
        inputs = xyz - wbounds[None][:, :3].to(xyz.device)
        inputs = inputs / ((wbounds[3:6] - wbounds[:3])[None]).to(xyz.device)
        inputs = torch.clamp(inputs, 0., 1-eps)
...
class HashgridNew(nn.Module):
    ...
    def forward(self, xyz, wbounds):
        # inputs = torch.clamp(xyz, min=wbounds[:3], max=wbounds[3:6])
        inputs = xyz - wbounds[None][:, :3]
        inputs = inputs / ((wbounds[3:6] - wbounds[:3])[None])
        inputs = torch.clamp(inputs, 0., 1-eps)
        feat = self.encoder(inputs, normalize=False)
        return feat

class TriPlaneNew(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
