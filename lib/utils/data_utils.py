...
def load_resize_bytes(imp: str, scale: float = 1., encode_ext: str = '.jpg', decode_flag: str = cv2.IMREAD_UNCHANGED):
    # copy from EasyVolumetricVideo
    # Author: Zhen Xu
    # Load image -> resize -> undistort -> save to bytes (jpeg)
    import ipdb; ipdb.set_trace()
    img = load_image_from_bytes(load_image_bytes(imp), decode_flag)  # cv2 decoding (fast)

    oH, oW = img.shape[:2]
    H, W = int(oH * scale), int(oW * scale)
    img = cv2.resize(img, (W, H), interpolation=cv2.INTER_AREA)  # H, W, 3, uint8

    is_success, buffer = cv2.imencode(encode_ext, img)
    buffer: np.ndarray
...
def load_npz_encode_bytes(im: str):
    seg = np.load(im)['arr_0']
    _, buffer = cv2.imencode('.png', seg.astype(np.uint8))
    buffer: np.ndarray
    buffer = BytesIO(buffer)  # is this slow? tobytes and BytesIO
    return buffer

def load_image_from_bytes(buffer: np.ndarray, ratio=1., normalize=True, decode_flag=cv2.IMREAD_UNCHANGED):
