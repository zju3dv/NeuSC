...
from lib.utils.colmap.read_write_model import read_model, qvec2rotmat
...
def main(args,
    ...
    train_split = [os.path.basename(cam_dict[k]['img_path']) for k in cam_dict]
    with open(join(input_path, 'annots/train.txt'), 'w') as f:
        f.write('\n'.join(train_split))
